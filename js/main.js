...
async function displayAlbums() {
    try {
        let fetchingSongs = await fetch(`https://github.com/DanielHashmi/Spotify-Clone/tree/main/Musics/`)
        let response = await fetchingSongs.text();
        let div = document.createElement('div');
        div.innerHTML = response;
        let anchors = div.getElementsByTagName('a');
        ...
        for (let i = 0; i < allAnch.length; i++) {
            if (allAnch[i].href.includes('/Musics') && !allAnch[i].href.includes('.htaccess')) {
                ...
                let fetchingSongs = await fetch(`https://github.com/DanielHashmi/Spotify-Clone/tree/main/Musics/${folders}/info.json`)
